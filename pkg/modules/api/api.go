...
func (a *Api) Descriptor() gotenberg.ModuleDescriptor {
	return gotenberg.ModuleDescriptor{
		...
		FlagSet: func() *flag.FlagSet {
			...
			fs.Duration("api-start-timeout", time.Duration(300)*time.Second, "Set the time limit for the API to start")
			fs.Duration("api-timeout", time.Duration(300)*time.Second, "Set the time limit for requests")
			fs.String("api-body-limit", "100MB", "Set the body limit for multipart/form-data requests - it accepts values like 5MB, 1GB, etc")
